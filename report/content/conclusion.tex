...
\begin{itemize}
    \item \textbf{Sự sụt giảm nghiêm trọng hiệu suất trên các lớp ít mẫu (Class Imbalance):} Phân tích ma trận nhầm lẫn tại Hình~\ref{fig:cm_all} trên các tập dữ liệu như CK+48, FERPlus, và SFEW2.0 cho thấy mô hình bị thiên lệch nặng nề về các lớp chiếm đa số như \textit{happy} và \textit{neutral}. Cụ thể, \textit{fear} đạt $0\%$ trên cả CK+48 và SFEW2.0, chỉ $4.4\%$ trên AffectNet và $8.3\%$ trên MMAFEDB. Tương tự, \textit{disgust} chỉ đạt $4.3\%$ trên SFEW2.0 và $18.5\%$ trên AffectNet (xem Bảng~\ref{tab:per_class_acc}). Hàm mất mát hiện tại chưa có cơ chế trọng số để bù trừ sự mất cân bằng này, dẫn đến Mean Accuracy thấp hơn Overall Accuracy $10$--$15\%$ trên hầu hết các tập.
    \item \textbf{Nhạy cảm với chất lượng và độ phân giải hình ảnh:} Đối với tập dữ liệu CK+48, do ảnh gốc có độ phân giải rất thấp ($48 \times 48$ pixel) nên khi phóng đại lên $224 \times 224$ để phù hợp đầu vào mô hình, các đặc trưng hình học và biểu cảm cơ mặt vi tế đã bị mờ nhòe. Hậu quả rõ rệt nhất là toàn bộ 25 mẫu \textit{fear} bị phân tán sang \textit{sadness} (10 mẫu) và \textit{happy} (4 mẫu) --- không một mẫu nào được phân loại đúng. Do CLIP được huấn luyện trên ảnh chất lượng cao, sự không khớp về chất lượng ảnh khiến mặt nạ Sigmoid khó định vị chính xác vùng đặc trưng cảm xúc.
    \item \textbf{Giới hạn của thông tin ảnh tĩnh (Static FER):} Mô hình CAFE chỉ xử lý thông tin trên từng khung ảnh độc lập. Trong thực tế, các biểu cảm khuôn mặt mang tính động và diễn tiến theo thời gian. Việc thiếu đi ngữ cảnh thời gian khiến mô hình dễ nhầm lẫn giữa các trạng thái cảm xúc chuyển tiếp hoặc mơ hồ về mặt ngữ nghĩa: trên CK+48, 28/45 mẫu \textit{anger} bị phân loại thành \textit{disgust} do hai biểu cảm này có chuyển động cơ vùng miệng và lông mày tương đồng khi chỉ xem xét ảnh tĩnh.
    \item \textbf{Sự phụ thuộc vào phân phối nhãn nguồn:} Mô hình được huấn luyện trên nhãn cứng của RAF-DB vốn được gán bởi các chuyên gia dựa trên phân phối chuẩn. Khi chuyển sang các tập dữ liệu có quy ước gán nhãn khác (như FERPlus sử dụng cơ chế bỏ phiếu đám đông để tạo nhãn phân phối), sự sai lệch trong định nghĩa biên cảm xúc làm giảm độ chính xác tổng thể, phản ánh qua chênh lệch $-6.35\%$ OA trên FERPlus so với bài báo gốc.
\end{itemize}
...
Đồ án môn học ``Nhận dạng mẫu'' với chủ đề Nhận dạng cảm xúc khuôn mặt có khả năng tổng quát hóa cao (Generalizable FER) đã đạt được các mục tiêu đề ra về nghiên cứu lý thuyết, tái hiện thực nghiệm và phát triển ứng dụng. Thông qua quá trình triển khai dự án, nhóm đã đạt được các kết quả cụ thể sau:
...
\begin{itemize}
    ...
    \item \textbf{Về mặt thực nghiệm:} Nhóm đã tái cài đặt và huấn luyện thành công mô hình CAFE trên tập dữ liệu nguồn RAF-DB, đồng thời thực hiện đánh giá chéo miền (zero-shot cross-domain) trên năm bộ dữ liệu đích bao gồm FERPlus, AffectNet, SFEW2.0, MMAFEDB và bộ dữ liệu mở rộng ngoài bài báo CK+48. Kết quả tái hiện trên RAF-DB ($89.15\%$ OA) và AffectNet ($52.01\%$ OA) vượt so với kết quả báo cáo tại Bảng 1 trong bài báo gốc \cite{zhang2024generalizable}, xác nhận tính đúng đắn của triển khai. Tuy nhiên, kết quả trên FERPlus ($66.81\%$, thấp hơn $6.35\%$) và SFEW2.0 ($46.87\%$, thấp hơn $5.99\%$) chưa đạt mức tham chiếu, phản ánh ảnh hưởng thực tế của sự khác biệt về cấu trúc dữ liệu và quy ước gán nhãn giữa các nguồn Kaggle và thiết lập gốc của tác giả.
...
Nhìn chung, đồ án này không chỉ giúp các thành viên trong nhóm củng cố vững chắc các kiến thức lý thuyết đã học trên lớp, mà còn rèn luyện kỹ năng đối mặt và phân tích các thách thức thực tế của học sâu như sự lệch miền dữ liệu (domain gap), chất lượng ảnh thấp, mất cân bằng phân phối lớp và sự không nhất quán giữa các giao thức đánh giá. Những kết quả đạt được, cùng với các hạn chế được ghi nhận trung thực và các hướng phát triển được đề xuất trong báo cáo, sẽ là nền tảng quan trọng giúp nhóm tiếp tục nghiên cứu sâu hơn về thị giác máy tính và trí tuệ nhân tạo trong tương lai.